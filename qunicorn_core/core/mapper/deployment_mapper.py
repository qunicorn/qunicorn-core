...
def deployment_dto_to_deployment(deployment: DeploymentDto) -> DeploymentDataclass:
    return DeploymentDataclass(
        ...
        program_list=[quantum_program_mapper.dto_to_quantum_program(qc) for qc in deployment.program_list],
...
def deployment_dto_to_deployment_without_id(deployment: DeploymentDto) -> DeploymentDataclass:
    return DeploymentDataclass(
        ...
        program_list=[quantum_program_mapper.dto_to_quantum_program_without_id(qc) for qc in deployment.program_list],
        deployed_at=deployment.deployed_at,
        name=deployment.name,
    )
...
def deployment_to_deployment_dto(deployment: DeploymentDataclass) -> DeploymentDto:
    return DeploymentDto(
        ...
        program_list=[quantum_program_mapper.quantum_program_to_dto(qc) for qc in deployment.program_list],
        deployed_at=deployment.deployed_at,
        name=deployment.name,
    )
