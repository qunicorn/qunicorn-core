...
class Qasm3ToBraket(CircuitTranspiler, source="QASM3", target="BRAKET", cost=2):
    ...
    def transpile_circuit(self, circuit: Any) -> Circuit:
        ...
        circuit = re.sub(
            r"^\s*cx(\s+[a-zA-Z0-9_]+(?:\[[0-9]+\])?\s*,\s+[a-zA-Z0-9_]+(?:\[[0-9]+\])?;)",
            r"cnot\1",
            circuit,
            flags=re.MULTILINE,
        )
        circuit = re.sub(
            r"^\s*ccx(\s+[a-zA-Z0-9_]+(?:\[[0-9]+\])?\s*,\s+[a-zA-Z0-9_]+(?:\[[0-9]+\])?\s*,\s+[a-zA-Z0-9_]+(?:\[[0-9]+\])?;)",
            r"ccnot\1",
            circuit,
            flags=re.MULTILINE,
        )
...
class BraketToQasm3(CircuitTranspiler, source="BRAKET", target="QASM3", cost=2):
    ...
    def transpile_circuit(self, circuit: Any) -> str:
        ...
        # TODO remove workaround once no longer required!
        qasm_str = re.sub(
            r"^\s*cnot(\s+[a-zA-Z0-9_]+(?:\[[0-9]+\])?\s*,\s+[a-zA-Z0-9_]+(?:\[[0-9]+\])?;)",
            r"cx\1",
            qasm_str,
            flags=re.MULTILINE,
        )
        qasm_str = re.sub(
            r"^\s*ccnot(\s+[a-zA-Z0-9_]+(?:\[[0-9]+\])?\s*,\s+[a-zA-Z0-9_]+(?:\[[0-9]+\])?\s*,\s+[a-zA-Z0-9_]+(?:\[[0-9]+\])?;)",
            r"ccx\1",
            qasm_str,
            flags=re.MULTILINE,
        )
